...
if ($setting_items_per_page != 0) {
	...
	$TEXT['PREVIOUS_PAGE'] = '<< '.$TEXT['PREVIOUS_PAGE'];
	$TEXT['NEXT'] = $TEXT['NEXT'].' >>';
	$TEXT['NEXT_PAGE'] = $TEXT['NEXT_PAGE'].' >>';
	...
	if ($position > 0) {
		...
		// Show text without links
		$previous_link = $TEXT['PREVIOUS'];
		$previous_page_link = $TEXT['PREVIOUS_PAGE'];
	}
	if ($position+$setting_items_per_page >= $total_num) {
		// Show text without links
		$next_link = $TEXT['NEXT'];
		$next_page_link = $TEXT['NEXT_PAGE'];
	} else {
		if (isset($_GET['g']) AND is_numeric($_GET['g'])) {
	...
	$display_previous_next_links = 'style="display:none;"';
	// Init vars so PHP throws no errors
	$previous_link = '';
	$previous_page_link = '';
	$next_link = '';
	$next_page_link = '';
...
if ($num_items > 0) {
	...
	while ($item = $query_items->fetchRow()) {
		...
		$item_date = date(DATE_FORMAT, $item['modified_when']);
		$item_time = date(TIME_FORMAT, $item['modified_when']);
		...
		if ($query_num_options->numRows() > 0) {
			while ($num_options = $query_num_options->fetchRow()) {
				...
				if ($query_attributes->numRows() > 0) {
					...
					$option_select .= "<select id=".$select_id." name='attribute[]'>\n"; 
		...
		if (isset($users[$uid]['username']) AND $users[$uid]['username'] != '') {
			$values = array(
				...
				$option, stripslashes($item['description']),
				stripslashes($item['full_desc']), 
				$setting_continue_url,
		...
		// Uncomment the following lines, if you use the classic Bakery table layout
